...
export async function getAdminDashboardOverview(
  supabase: BCareSupabaseClient,
): Promise<{ data: AdminDashboardOverview | null; error: { message: string } | null }> {
  ...
  const [bookingsResult, servicesResult, repairJobsResult, productsResult] =
    await Promise.all([
      supabase
        .from("bookings")
        .select("booking_date, status, service_id")
        .gte("booking_date", fourteenDaysAgoStr)
        .in("status", ["confirmed", "completed"]),
      supabase.from("services").select("id, base_price"),
      supabase
        .from("repair_jobs")
  ...
  if (repairJobsResult.error) {
    return { data: null, error: repairJobsResult.error };
  }
  ...
  const priceByServiceId = new Map(
    (servicesResult.data ?? []).map((service) => [service.id, service.base_price]),
  );

  // สร้างรายการ 14 วันล่าสุดไว้ก่อน (ให้ครบทุกวันแม้ไม่มียอด จะได้กราฟไม่ขาดช่วง)
  ...
  for (const booking of bookingsResult.data ?? []) {
    const price = priceByServiceId.get(booking.service_id) ?? 0;
    const current = revenueByDate.get(booking.booking_date) ?? 0;
    revenueByDate.set(booking.booking_date, current + price);
  ...
  const lowStockProducts = (productsResult.data ?? []).slice(0, 5);

  return {
    data: {
      bookingsToday,
      dailyRevenue,
      lowStockCount: productsResult.data?.length ?? 0,
      lowStockProducts,
      pendingRepairJobCount: repairJobsResult.count ?? 0,
      revenueToday,
    },
    error: null,
  };
