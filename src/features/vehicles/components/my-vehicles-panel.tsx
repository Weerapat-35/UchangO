...
function emptyValues(): VehicleFormValues {
  return { brand: "", color: "", licensePlate: "", model: "", year: "" };
}
...
function AddVehicleForm({
  ...
  const [values, setValues] = useState<VehicleFormValues>(emptyValues());
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (validationError) {
      setCreateState({ error: validationError, message: null, status: "error" });
      return;
    }
    ...
    const { data, error } = await createVehicle(supabase, {
      brand: values.brand,
      color: values.color,
      customerId,
  ...
  return (
    <section className="mb-5 rounded-lg border border-[var(--line)] bg-white p-5 shadow-sm">
      <p className="text-sm font-semibold text-[var(--brand)]">+ Add vehicle</p>
      <p className="mt-1 text-sm leading-6 text-[var(--muted)]">
        Add a vehicle manually instead of waiting for a booking to create one.
      </p>
      ...
      <form className="mt-4 grid gap-4 sm:grid-cols-2" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-3 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between">
          <button
            ...
            type="submit"
          >
            {createState.status === "saving" ? "Adding..." : "Add vehicle"}
          </button>
...
export function MyVehiclesPanel() {
  ...
  function handleVehicleCreated(newVehicle: Vehicle) {
    if (loadState.status !== "ready") {
      return;
    }
    ...
    setLoadState({
      ...loadState,
      vehicles: [newVehicle, ...loadState.vehicles],
    });
  }
